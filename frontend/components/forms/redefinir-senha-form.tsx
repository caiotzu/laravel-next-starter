...
} from "@/lib/validations/auth/redefinir-senha-schema"
import { getPasswordRequirements } from "@/lib/validations/password"

interface RedefinirSenhaFormProps {
  onSubmit: (
    data: RedefinirSenhaData,
    setError: UseFormSetError<RedefinirSenhaData>
  ) => Promise<void>
  className?: string
...
export function RedefinirSenhaForm({
  ...
  async function handleFormSubmit(data: RedefinirSenhaData) {
  ...
  useEffect(() => {
    ...
    Object.entries(fieldErrors).forEach(([field, messages]) => {
      ...
      setError(field as keyof RedefinirSenhaData, {
        type: "server",
        message: messages[0],
      })
    })
