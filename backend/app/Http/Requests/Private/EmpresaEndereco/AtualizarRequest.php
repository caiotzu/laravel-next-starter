...
namespace App\Http\Requests\Private\EmpresaEndereco;
...
class AtualizarRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $enderecoId = $this->route('enderecoId');

            $endereco = EmpresaEndereco::query()
                ->where('id', $enderecoId)
            ...
            if (!$endereco) {
                $validator->errors()->add(
                    'endereco',
                    'O endereço informado não pertence à empresa.'
                );
